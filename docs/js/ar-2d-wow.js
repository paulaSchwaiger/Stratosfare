...
function setFooterMode(mode) {
 const pinsView = document.getElementById("footer-view-pins");
  ...
  function initARLogic() {
        ...
        // -------------------------------------------------------
        // Aspect Fix (gegen "gequetscht" / falsches Seitenverhältnis)
        // -------------------------------------------------------
        const fixARAspect = () => {
        ...
        // -------------------------------------------------------
        // Mission Controller (DEBUG: nur Mission 2)
        // -------------------------------------------------------
        
        // ---- HARD KILL: alles Pin/Overlay/Progress weghauen ----
        const killMission1UI = () => {
            // Pins/PinGroups komplett ausblenden (egal ob vorhanden)
            for (let i = 1; i <= 10; i++) {
            document.getElementById(`pinGroup-${i}`)?.setAttribute("visible", "false");
            document.getElementById(`pin-${i}`)?.setAttribute("visible", "false");
            document.getElementById(`hit-${i}`)?.setAttribute("visible", "false");
            }
        ...
        // Progressbar auf 0
        const bar = document.querySelector("#ar-footer .footer-progress-bar");
        if (bar) bar.style.width = "0%";

        // Footer Button disabled (optional)
        const btn = document.querySelector("#ar-footer .footer-cta");
        if (btn) {
        btn.classList.add("is-disabled");
        btn.textContent = "MISSION 2";
        btn.onclick = null;
        }
    };
    ...
    const startMission2Only = () => {
        ...
        // Minigame öffnen (deine Funktion)
        setupMission2Minigame({
        onDone: () => console.log("✅ Mission 2 done"),
        });
    ...
    document.getElementById("launch-btn")?.addEventListener("click", () => {
        // hier deine Launch-Animation oder Countdown etc.
        launchRocket(); // oder was du schon hast
    });
        ...
        // ✅ nur Mission 2 starten
        if (!missionStarted) startMission2Only();
        });
        ...
        /* nur für Minigame debug kurz raus
        // -------------------------------------------------------
        // Mission Controller
        // -------------------------------------------------------
        let missionStarted = false;
        ...
        const startMission2 = () => {
            console.log("🚀 START Mission 2 (Minigame)");

            setupMission2Minigame({
            onDone: () => {
                console.log("✅ Mission 2 done");
                // später: Launch + Redirect oder nächster Schritt
                // launchRocket();
            },
            });
        };
        ...
        const startMission1 = () => {
            console.log("🧩 START Mission 1 (Pins)");
            ...
            // Debug: Minigame direkt testen ohne Pins (URL: ?minigame=1)
            const params = new URLSearchParams(window.location.search);
            if (params.get("minigame") === "1") {
            startMission2();
            }
        };
        ...
        marker.addEventListener("markerFound", () => {
            ...
            // WOW: "kommt aus Podest" (optional)
            // Falls du schon eine Position hast, die passt, setz vorher die Startposition:
            // rocket.setAttribute("position", "0 0 0");
            rocket.setAttribute(
            "animation__rise",
            "property: position; dur: 900; easing: easeOutBack; to: 0 0.35 0"
            );

            loadingEl.classList.add("hidden");
            arFooter.classList.remove("hidden");

            // Mission 1 nur einmal starten (nicht bei jedem Re-Focus)
            if (!missionStarted) startMission1();
        });
  ...
  // Mission 1 Fortschritt
  const clickedPins = new Set();
  ...
  // Inhalte (für wow später icon + subtitle; aktuell title/text ok)
  const pinContent = [
    { title: "MISSION & VISION", text: "..." },
    { title: "NETZWERK & PARTNERSCHAFTEN", text: "..." },
    { title: "TEAM & ARBEITSWEISE", text: "..." },
    { title: "PROJEKTE", text: "..." },
  ];
  ...
  // Klick: auf hitbox -> overlay öffnen
  hitTargets.forEach((hit, idx) => {
    if (!hit) return;
    hit.addEventListener("click", () => openInfoByIndex(idx));
  });

  // Button: Mission 2 starten (erst wenn alle Pins geklickt)
  startBtn?.addEventListener("click", (e) => {
    ...
    // Mission 1 "abschließen": Pins ausblenden oder stehen lassen
    // (du wolltest später Missionsgefühl -> hier ausblenden)
    pinGroups.forEach((g) => g?.setAttribute("visible", "false"));
...
function setupMission2Minigame({ goal = 30, durationMs = 5000, onDone } = {}) {
  ...
  const finish = (success) => {
    ...
    if (success) {
      statusEl.textContent = `Geschafft! ${taps}/${goal} Taps 🎉`;
