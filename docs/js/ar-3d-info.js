...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function createARScene() {
    return `
      ...
      <!--  MARKER -->
        ...
        <!-- Cursor/Raycaster fr Hover mit Maus -->
       <a-entity
          id="cam"
          camera
  ...
  function fitPodestToRealDims(el, { height = 0.95, diameter = 1.8 } = {}) {
    ...
    // Nimm den gr”áeren der beiden Faktoren (damit es nicht zu klein wird)
  ...
  else scene.addEventListener("loaded", resizeAR, { once: true });
    ...
    const enableTouchToMouseWithCoords = () => {
      ...
      canvas.addEventListener(
        ...
        (e) => {
          ...
          // wichtig: mousemove setzen, damit A-Frame weiá "wo" der Cursor ist
  ...
  const disableARInteraction = () => {
    ...
    // Overlay schlieáen (falls noch offen)
    ...
    const pinContent = [
      ...
      { title: "NETZWERK & PARTNERSCHAFTEN", 
        text: "Groáe Missionen gelingen nie allein. Wenn Unternehmen, Start-ups und Forschung zusammenkommen, entsteht echte Aufbruchsstimmung." },
      ...
      { title: "PROJEKTE", 
        text: "Ideen heben ab, wenn man sie teilt. In gemeinsamen Projekten werden Visionen getestet, gesch„rft und umgesetzt." },
    ];
    ...
    let completedPins = 0; 
    let openedPinIndex = null;  // welcher Pin hat das Overlay ge”ffnet?
  ...
  const startCountdown = (seconds = 3, onDone) => {
    ...
    const unlockNextPinIfNeeded = () => {
      ...
      // nur wenn der User den "neuesten" (aktuellen) Pin ge”ffnet hatte, wird der n„chste Pin freigeschaltet
