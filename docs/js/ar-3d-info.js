...
function toggleLang() {
  ...
  apply3DTranslations();
   // only if these exist globally
  if (window.openedPinIndex != null && typeof window.openInfoByIndex === "function") {
    window.openInfoByIndex(window.openedPinIndex);
  }
...
if (scene.hasLoaded) setupCanvasPick();
  ...
  allowBtn?.addEventListener("click", async () => {
    try {
      ...
      apply3DTranslations(); // first attempt
      const scene = document.getElementById("ar-scene");
      if (scene?.hasLoaded) initARLogic();
      else scene?.addEventListener("loaded", initARLogic, { once: true });
      
      scene?.addEventListener("loaded", apply3DTranslations, { once: true }); // second attempt
